...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM loaded, setting initial Item No.');
    ...
    if (firstItem) {
        ...
        if (firstItemNoInput) {
            ...
            console.error('Item No. input not found in first item');
        }
    } else {
...
document.getElementById('addItem').addEventListener('click', () => {
    ...
    if (!firstItem) {
        console.error('No item to clone');
        return;
    }
    ...
    if (newItemNoInput) {
        ...
        console.error('Item No. input not found in new item');
    }

    itemsContainer.appendChild(newItem);
    console.log('New item added with Item No:', currentItemCount + 1);
});
...
document.addEventListener('click', (e) => {
    if (e.target.classList.contains('remove-item')) {
        ...
        if (items.length > 1) {
            ...
            console.log('Item removed, renumbered remaining items');
        } else {
...
document.getElementById('packingListForm').addEventListener('submit', (e) => {
    ...
    const action = e.submitter ? e.submitter.value : 'csv'; // Fallback to 'csv' if submitter is undefined
