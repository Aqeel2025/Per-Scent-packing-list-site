...
if (typeof document === 'undefined') {
    console.error('Error: script.js is running in a non-browser environment (e.g., Node.js). This script requires a browser DOM.');
    module.exports = {}; // Prevent Node.js module errors
} else {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!firstItem) {
            ...
            alert('Error: Initial item not found. Please check index.html structure.');
            return;
        }
        ...
        if (!firstItemNoInput) {
            ...
            alert('Error: Item No. input missing in initial item.');
            return;
        }
    ...
    if (addItemButton) {
        addItemButton.addEventListener('click', () => {
            ...
            if (!itemsContainer) {
                ...
                alert('Error: Items container not found. Please check index.html structure.');
                return;
            }
            const firstItem = itemsContainer.querySelector('.item');
            if (!firstItem) {
                console.error('Error: No .item element to clone');
                alert('Error: No item template found. Please check index.html structure.');
                return;
            }
            try {
        ...
        console.error('Error: Element with id="addItem" not found in DOM');
        alert('Error: Add Another Item button not found. Please check index.html for <button id="addItem">.');
    }
    ...
    if (carrierSelect) {
        carrierSelect.addEventListener('change', (e) => {
            ...
            const otherCarrierInput = otherCarrierLabel ? otherCarrierLabel.querySelector('input') : null;
            if (otherCarrierLabel && otherCarrierInput) {
                ...
                console.error('Error: #otherCarrierLabel or its input not found in DOM');
            }
        });
    } else {
        console.error('Error: #carrierSelect not found in DOM');
    }
            ...
            if (action === 'csv') {
                ...
                try {
                    ...
                    const errorMessage = isChrome
                        ? `Error generating PDF: ${error.message}. In Chrome, try disabling ad blockers (e.g., uBlock Origin), clearing cache (Settings > Privacy > Clear browsing data), or testing in incognito mode (Ctrl+Shift+N). Ensure lib/jspdf.plugin.autotable.min.js exists in the project directory.`
                        : `Error generating PDF: ${error.message}. Check your internet connection, ensure lib/jspdf.plugin.autotable.min.js exists, or try another browser.`;
                    alert(errorMessage);
                }
            }
        });
    } else {
        console.error('Error: #packingListForm not found in DOM');
        alert('Error: Form not found. Please check index.html for <form id="packingListForm">.');
    }
}
